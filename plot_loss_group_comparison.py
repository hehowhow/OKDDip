...
for label, filename in csv_files.items():
    try:
        ...
        loss_group = df['test/loss_group'].values
        data_dict[label] = {'epochs': epochs, 'loss_group': loss_group}
...
ax.set_ylabel('Test Loss Group', fontsize=22, fontweight='bold')
# ax.set_title('Test Loss Group vs Epoch for Different λ Values', 
...
if data_dict:
    ...
    for data in data_dict.values():
        all_losses.extend(data['loss_group'])
...
# 保存图片
plt.savefig('loss_group_comparison.png', dpi=300, bbox_inches='tight')
plt.savefig('loss_group_comparison.pdf', bbox_inches='tight')

# 显示图形
plt.show()

print("\n图表已保存为 'loss_group_comparison.png' 和 'loss_group_comparison.pdf'")
...
for label, data in data_dict.items():
    ...
    print(f"  Loss Group范围: {loss_group.min():.6f} - {loss_group.max():.6f}")
    print(f"  最终Loss Group: {loss_group[-1]:.6f}")
    print(f"  平均Loss Group: {loss_group.mean():.6f}")
    
    # 计算收敛趋势（最后10%的平均值）
    last_10_percent = int(len(loss_group) * 0.1)
    if last_10_percent > 0:
        final_avg = loss_group[-last_10_percent:].mean()
